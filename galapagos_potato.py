...
update 03/05/2021: allow Stochastic Gradient Decent (50% of data) for faster training time per epochs. Updated on train_network() use 
stochastic=False for full gradient decent

update 03/05/2021: change error to use cross entropy error -- DONE but Math Domain Error due to log() and exp()
update 07/05/2021: set domain initiation to try to solve Math Domain Error
'''
...
def sigmoid(weighted_input):
    if weighted_input < 0:
        return 1 - 1 / (1 + exp(weighted_input))
    ...
    for i in reversed(range(len(network))):
        ...
        if i == len(network) - 1: # output layer
            for j in range(len(layer)):
                ...
                neuron['delta'] = (-neuron['output']*(1-expected[j]) + expected[j]*(1-neuron['output']))/(neuron['output']*(1-neuron['output']))
        else: # hidden layer
            for j in range(len(layer)):
                error = 0
...
def train_network(network, x_train, y_train, learning_rate, epochs, n_outputs, stochastic=True):
    
    for epoch in range(epochs):
        sum_error = 0

        if stochastic:
            zip_dataset = list(zip(x_train, y_train))
            shuffle(zip_dataset)
            shuff_x, shuff_y = zip(*zip_dataset)
            shuff_x = list(shuff_x[:len(shuff_x)//2])
            shuff_y = list(shuff_y[:len(shuff_y)//2])

        else:
            shuff_x = x_train
            shuff_y = y_train

        for n in range(len(shuff_y)):
            x = shuff_x[n]
            y = shuff_y[n]
            ...
            # print(outputs)
            sum_error += sum([expected[i]*log(outputs[i]) + (1-expected[i])*log(1-outputs[i]) for i in range(len(expected))])
            backward_propagate_error(network, expected)
            update_weights(network, x, learning_rate)
        print('epoch=',epoch+1, ', ', 'lrate=',learning_rate, ', ', 'error=', -sum_error/(len(shuff_y)))
...
if __name__ == "__main__":
    ...
    train_network(network, dataset, y, 0.05, 100, n_outputs)
